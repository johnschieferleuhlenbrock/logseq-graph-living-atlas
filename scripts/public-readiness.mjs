...
function checkGitShape() {
  ...
  if (branch !== "main") findings.push(`git: current branch must be main, found ${branch || "(detached)"}`);
}
...
function checkPublicRepoScaffolding() {
  ...
  if (release) {
    for (const command of ["npm run validate", "npm run smoke:package", "npm run check:release", "npm publish --access public --provenance"]) {
